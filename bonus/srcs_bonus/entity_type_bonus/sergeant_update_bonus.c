...
{
	if (sprite->animation == textures[IDX_TXTR_SERGEANT_WALK])
...
static bool	sergeant_range_attack(t_entity *entity, t_sprite *sprite, t_game *game)
{
	if (entity->cooldown > 0)
		return (true);
	sprite->next_animation = game->textures[IDX_TXTR_SERGEANT_WALK];
	sprite->animation = game->textures[IDX_TXTR_SERGEANT_ATTACK];
