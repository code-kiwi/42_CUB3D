...
static bool	arch_vile_get_killed(t_game *game, t_entity *entity)
{
	...
	entity->sprite->animation = game->textures[IDX_TXTR_ARCH_VILE_DEATH];
...
{
	entity->sprite->next_animation = game->textures[IDX_TXTR_ARCH_VILE_WALK];
	entity->sprite->animation = game->textures[IDX_TXTR_ARCH_VILE_PAIN];
...
bool	arch_vile_init(t_entity *entity, t_list *textures[MAP_NB_IDS])
{
	...
	entity->type = NULL;
	t_sprite_init(entity->sprite, textures[IDX_TXTR_ARCH_VILE_WALK], WIN_HEIGHT);
	return (true);
}
