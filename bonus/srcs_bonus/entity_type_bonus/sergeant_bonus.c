...
static bool	sergeant_get_killed(t_game *game, t_entity *entity)
{
	...
	entity->sprite->animation = game->textures[IDX_TXTR_SERGEANT_DEATH];
...
{
	entity->sprite->next_animation = game->textures[IDX_TXTR_SERGEANT_WALK];
	entity->sprite->animation = game->textures[IDX_TXTR_SERGEANT_PAIN];
...
bool	sergeant_init(t_entity *entity, t_list *textures[MAP_NB_IDS])
{
	...
	entity->type = NULL;
	t_sprite_init(entity->sprite, textures[IDX_TXTR_SERGEANT_WALK], WIN_HEIGHT);
	return (true);
}
