...
{
	if (sprite->animation == textures[IDX_TXTR_ARCH_VILE_WALK])
		sprite->animate = false;
}

static void	arch_vile_close_attack(t_entity *entity, t_sprite *sprite, t_game *game)
{
	if (entity->cooldown > 0)
		return ;
	sprite->next_animation = game->textures[IDX_TXTR_ARCH_VILE_WALK];
	sprite->animation = game->textures[IDX_TXTR_ARCH_VILE_ATTACK];
