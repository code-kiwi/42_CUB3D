...
{
	if (sprite->animation == &animations[IDX_TXTR_PAIN_ELEM_WALK])
...
static void	pain_elem_close_attack(t_entity *entity, t_sprite *sprite, t_game *game)
{
	if (entity->cooldown > 0)
		return ;
	sprite->next_animation = &game->animations[IDX_TXTR_PAIN_ELEM_WALK];
	sprite->animation = &game->animations[IDX_TXTR_PAIN_ELEM_ATTACK];
