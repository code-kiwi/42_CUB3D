...
static bool	caco_get_killed(t_game *game, t_entity *entity)
{
	...
	set_animation(entity->sprite, &game->anim[IDX_TXTR_CACO_DEATH]);
...
{
	set_animation(entity->sprite, &game->anim[IDX_TXTR_CACO_PAIN]);
	entity->sprite->next_animation = &game->anim[IDX_TXTR_CACO_WALK];
