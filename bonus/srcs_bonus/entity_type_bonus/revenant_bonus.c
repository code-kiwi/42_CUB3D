...
static bool	revenant_get_killed(t_game *game, t_entity *entity)
{
	...
	entity->sprite->animation = &game->animations[IDX_TXTR_REVENANT_DEATH];
...
{
	entity->sprite->next_animation = &game->animations[IDX_TXTR_REVENANT_WALK];
	entity->sprite->animation = &game->animations[IDX_TXTR_REVENANT_PAIN];
...
bool	revenant_init(t_entity *entity, t_animation animations[MAP_NB_IDS])
{
	...
	entity->type = NULL;
	t_sprite_init(entity->sprite, &animations[IDX_TXTR_REVENANT_WALK], WIN_HEIGHT);
	return (true);
}
