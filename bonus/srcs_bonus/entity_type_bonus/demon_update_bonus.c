...
{
	if (sprite->animation == &animations[IDX_TXTR_DEMON_WALK])
...
static void	demon_attack(t_entity *entity, t_sprite *sprite, t_game *game)
{
	if (entity->cooldown > 0)
		return ;
	sprite->next_animation = &game->animations[IDX_TXTR_DEMON_WALK];
	sprite->animation = &game->animations[IDX_TXTR_DEMON_ATTACK];
